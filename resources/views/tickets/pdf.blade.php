...
<head>
    <meta charset="utf-8">
    <title>{{ __('messages.tickets.pdf.title', ['code' => $registration->booking_code]) }}</title>
    <style>
...
<body>
    <div class="ticket-box">
        <div class="header">
            <span class="title">{{ __('messages.tickets.pdf.header_title') }}</span>
            <span class="status-badge">{{ __('messages.tickets.pdf.status_valid') }}</span>
            <div class="clear"></div>
        </div>

        <div class="info-row">
            <span class="label">{{ __('messages.tickets.pdf.booking_code') }}</span>
            <strong style="font-size: 16px;">{{ $registration->booking_code }}</strong>
        </div>

        <div class="info-row">
            <span class="label">{{ __('messages.tickets.pdf.tour_title') }}</span>
            {{ $registration->visit->visitType->title }}
        </div>

        <div class="info-row">
            <span class="label">{{ __('messages.tickets.pdf.location') }}</span>
            {{ $registration->visit->visitType->place->name }}
        </div>

        <div class="info-row">
            <span class="label">{{ __('messages.tickets.pdf.date') }}</span>
            {{ \Carbon\Carbon::parse($registration->visit->visit_date)->format('l, F j, Y') }}
        </div>

        <div class="info-row">
            <span class="label">{{ __('messages.tickets.pdf.start_time') }}</span>
            {{ \Carbon\Carbon::parse($registration->visit->effective_start_time ?? $registration->visit->visitType->start_time)->format('g:i A') }}
        </div>

        <div class="info-row">
            <span class="label">{{ __('messages.tickets.pdf.meeting_point') }}</span>
            {{ $registration->visit->visitType->meeting_point }}
        </div>

        <div class="info-row">
            <span class="label">{{ __('messages.tickets.pdf.num_participants') }}</span>
            {{ $registration->num_participants }}
        </div>

        <div class="info-row">
            <span class="label">{{ __('messages.tickets.pdf.booked_by') }}</span>
        ...
        <div class="qr-code">
            ...
            <br>
            <small style="color: #999;">{{ __('messages.tickets.pdf.scan_to_verify') }}</small>
        </div>

        <div class="footer">
            {!! __('messages.tickets.pdf.footer_text', ['year' => date('Y')]) !!}
        </div>
    </div>
</body>
</html>
