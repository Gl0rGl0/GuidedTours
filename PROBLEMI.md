...
| **35** | Home Page | Match Sys/Real | **Mancanza personalizzazione**: Home non cambia base ruolo. | ✅ **Resolved** | `home.blade.php`: Messaggio hero e CTA personalizzati per Admin (`admin.configurator`), Guide (`volunteer.availability.form`) e Customer (`user.dashboard`). |
...
| **37** | Assigned Visits | Help Users | **Assenza indicazioni operative**: Manca guida per i volontari. | ✅ **Resolved** | `tours/customized_visits.blade.php`: Alert `alert-info` con 4 istruzioni operative (mansioni, verifica biglietti, ecc.) visibile solo se `hasRole('Guide')`. |
| **38** | Assigned Visits | Aesthetics | **Ridondanza nome volontario**: Mostrato nome anche se assegnato a me. | ✅ **Resolved** | `components/tour-card.blade.php`: Condizione `!($visit->assignedVolunteer->getKey() == Auth::id() && hasRole('Guide'))` nasconde il proprio nome nella vista archivio. |
