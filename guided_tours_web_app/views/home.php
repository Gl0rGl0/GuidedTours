...
<?php
// Include DB connection only if not already included (e.g., by index.php)
// Note: A more robust solution might involve a central bootstrap file.
if (!isset($pdo)) {
...
try {
    ...
    // Use the existing $pdo connection if available
    if (isset($pdo)) {
        $stmt = $pdo->query($sql);
        $available_tours = $stmt->fetchAll();
    } else {
        // This case should ideally not happen if db_connect is included properly before view loading
        $error_message = "Database connection not found.";
    }


} catch (\PDOException $e) {
    error_log("Error fetching available tours on home page: " . $e->getMessage());
...
<?php else: ?>
    ...
    <ul class="tour-list">
        <?php foreach ($available_tours as $tour): ?>
            <li class="tour-item">
                <h4><?php echo htmlspecialchars($tour['visit_type_title']); ?></h4>
                ...
                <?php endif; ?>
                <?php /* Only show registration link if logged in and tour is proposed? Needs clarification
                <?php if (isset($_SESSION['user_id']) && $tour['status'] === 'proposed'): ?>
                ...
                */ ?>
                 <?php if ($tour['status'] === 'proposed'): ?>
                     <p><a href="index.php?page=register_tour&visit_id=<?php echo $tour['visit_id']; ?>">Register Interest</a></p>
