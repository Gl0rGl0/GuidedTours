...
\begin{itemize}
    ...
    \item \textbf{Meccanismi di conferma (Modal)}: I \textit{confirmation modals} per le azioni distruttive, al posto di impiegare librerie più pesanti come Livewire, sono stati realizzati tramite \textbf{Modali Bootstrap} nativi interfacciati con script JavaScript centralizzati. Questo rende più performante la pagina: uno script globale cattura l'evento, estrae dinamicamente i parametri utente/oggetto (\texttt{data-action}, \texttt{data-item-name}) e inietta i dati prima dell'apertura visiva, garantendo modularità.
\end{itemize}

\section{Design System Documentation}
Il restyling grafico ha posto forte enfasi sull'accessibilità, cercando di risolverne un difetto di base emerso nell'analisi. Il Design System finale si basa su una palette cromatica precisa e facilmente riconoscibile:
\begin{itemize}
    \item \textbf{Primary Blue (\#0d6efd)}: Utilizzato per le azioni principali (le classiche call-to-action), per i link interattivi e i pulsanti di prima grandezza. Rappresenta l'identità visiva del brand.
    \item \textbf{Light Blue / Subtle Background (\#cfe2ff)}: Impiegato per badge di stato e sfondi secondari delle card. Fornisce un contrasto bilanciato che alleggerisce l'impatto visivo senza affaticare la lettura del testo scuro in sovraimpressione.
    \item \textbf{Dark Grey (\#333333)}: Utilizzato come colore di rottura, specialmente nei contesti del tema scuro e per la tipografia forte dell'header, distinguendo i container e le card primarie dallo sfondo monocromo dell'applicazione.
\end{itemize}
L'impiego coerente di queste tre tonalità principali ha risolto in gran parte le incongruenze della \textit{Valutazione Euristica} (come stati informativi che utilizzavano semanticamente tinte fuorvianti, ad esempio il "Proposed" in giallo).
...
\begin{itemize}
    ...
    \item \textbf{Confirmation Trigger (Modali antierrore distruttivo)}: Onde scongiurare drammatici \textit{slips} neuromotori o errori d'azione distratta nel Backoffice (es. l'annullamento di risorse storicizzate o ban di un volontario), l'ecosistema frappone un baluardo esplicito. Il "Global Delete Modal" intercetta la richiesta ed obbliga l'agente ad una validazione esplicita conscia ("This action cannot be undone").
    \item \textbf{Continuous / Asynchronous Scrolling (Load More)}: In contrapposizione ad una impaginazione classica a scaglioni chiusi (che affievolirebbe il ritmo e sfalderebbe il contesto esplorativo), l'utilizzo del pattern asincrono incrementale alla base dell'elenco tour consente un'immersione progressiva nel catalogo, conservando intatto l'header e il form dei filtri.
    \item \textbf{Live Input Feedback (Validazione Contestuale)}: Dislocato nei form critici. Comunica istantaneamente il superamento - o fallimento - dei vincoli strutturati (per es. la metrica \textit{real-time} della password). Stempera in radice la consueta ansia e frustrazione imputabili ai secchi rigetti in fase di sottomissione (Submission Postponed Errors). 
\end{itemize}
...
Sulla Home Page pubblica, il pannello di ricerca esplorativo che accoglie i biglietti da acquistare integra funzioni multiple: ricerca testuale live, selezione del luogo fisico (filtro location), costrizione del costo ("Free Only") ed un selettore di ordinamento (temporale o alfabetico). Questa architettura è pilotata integramente in \textbf{Alpine.js}, il quale intercetta l'evento di sottomissione assemblando i dati in querystring e richiedendo l'aggiornamento tramite chiamate AJAX. Una volta ricevuta la preview dal backend, il nuovo snippet HTML sostiuisce unicamente la griglia evitando ricaricamenti molesti e curando la storia del browser inserendo lo stato nella location bar reale (\texttt{pushState}).

\subsection{Pattern di Paginazione}
Per limitare la profusione cognitiva emersa nelle liste particolarmente corpose dei tour, la paginazione ibrida assorbe il mix preferito tra la consueta Classic Pagination e un bottone asincrono "Load More". Con un'anima reattiva definita sempre in Alpine.js / Vanilla JS, ogni click adombra temporaneamente la view (stato di loading visivo del pulsante) procedendo a richiedere il frammento mancante da accodare. La vista viene poi delicatamente ricollocata allo zero della sezione di pertinenza via \texttt{scrollIntoView()}, eludendo qualsiasi disorientamento spaziale.

\subsection{Sistema di sicurezza (Modal) nell'Admin}
Per combattere ed azzerare gli accidentali slip inerenti alle routine irreversibili all'interno dell'Area di Backoffice (es: rimozione luoghi turistici, eliminazione formati d'evento in archivio o espulsione attori dal sito), l'ecosistema accorpa un \textbf{Global Delete Confirmation Modal}. Alla pressione del cestino sui record in tabella, l'azione non raggiunge immediatamente la root di distruzione; in luogo, viene invocato forzatamente il pop-up Bootstrap (bloccato come layout esterno) con l'avvertimento imperativo in rosso "This action cannot be undone". L'ostacolo esplicito previene qualsiasi errore muscolare di velocità della persona che cura interfacce troppo fitte.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/v2/v2_admin_planning.png}
    \hspace{0.04\textwidth}
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/v2/v2_add_visit.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/v2/v2_guide_visits.png}
    \hspace{0.04\textwidth}
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/v2/v2_guide_calendar.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/p1.png}
    \hspace{0.04\textwidth}
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/v2/p3_v2.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/p3.png}
    \hspace{0.04\textwidth}
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/v2/p6_v2.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/p11.png}
    \hspace{0.04\textwidth}
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/v2/p15_v2.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/p17.png}
    \hspace{0.04\textwidth}
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/v2/p19_v2.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/p21.png}
    \hspace{0.04\textwidth}
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/v2/p23_v2.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/p24.png}
    \hspace{0.04\textwidth}
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/v2/p26_v2.png}
    \caption{Fix P.26 -- Affordance "My Booking": V1 a sinistra (menu a tendina ambiguo), V2 a destra (bottone esplicito "Cancel Booking" con modale di conferma).}
    \label{fig:fix_p26}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/p29.png}
    \hspace{0.04\textwidth}
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/v2/p31_v2.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/p31.png}
    \hspace{0.04\textwidth}
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/v2/p33_v2.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/p2.png}
    \hspace{0.04\textwidth}
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/v2/p4_v2.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/p4.png}
    \hspace{0.04\textwidth}
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/v2/p7_v2.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/p6.png}
    \hspace{0.04\textwidth}
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/v2/p3_v2.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/p7.png}
    \hspace{0.04\textwidth}
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/v2/p9_v2.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/p8.png}
    \hspace{0.04\textwidth}
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/v2/p10_v2.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/p9.png}
    \hspace{0.04\textwidth}
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/v2/p11_v2.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/p10.png}
    \hspace{0.04\textwidth}
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/v2/p12_v2.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/p12.png}
    \hspace{0.04\textwidth}
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/v2/p13_v2.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/p33.png}
    \hspace{0.04\textwidth}
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/v2/p35_v2.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/p14.png}
    \hspace{0.04\textwidth}
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/v2/p16_v2.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/p16.png}
    \hspace{0.04\textwidth}
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/v2/p18_v2.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/p18.png}
    \hspace{0.04\textwidth}
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/v2/p20_v2.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/p19.png}
    \hspace{0.04\textwidth}
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/v2/p21_v2.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/p22.png}
    \hspace{0.04\textwidth}
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/v2/p24_v2.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/p23.png}
    \hspace{0.04\textwidth}
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/v2/p25_v2.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/p30.png}
    \hspace{0.04\textwidth}
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/v2/p32_v2.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/p32.png}
    \hspace{0.04\textwidth}
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/v2/p34_v2.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/p34.png}
    \hspace{0.04\textwidth}
    \includegraphics[width=0.47\textwidth, keepaspectratio]{capitoli/immagini/v2/p40_v2.png}
