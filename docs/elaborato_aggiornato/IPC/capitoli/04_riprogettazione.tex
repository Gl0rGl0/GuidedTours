...
La priorità degli interventi è stata determinata in base al \textbf{grado di severità} assegnato nella fase di debriefing (scala Nielsen 0--4). Sono stati affrontati tutti i problemi di grado \textbf{4} e la quasi totalità di quelli di grado \textbf{3}, la maggior parte di quelli di grado \textbf{2} e un sottoinsieme di quelli di grado \textbf{1} per i quali l'impatto sull'esperienza delle \textit{personas} lo giustificasse. I problemi rimasti aperti sono documentati nella Sezione~\ref{sec:problemi_aperti}.
...
\begin{itemize}
    ...
    \item \textbf{Feedback Semantico (Success \#198754, Danger \#dc3545, Info \#0dcaf0):} Questi colori seguono la convenzione cromatica universale e sono utilizzati per i \textit{Live Input Feedback} (es. validazione dei form) e per segnalare i cambiamenti di stato critici (es. il \textit{Global Delete Modal} in rosso), rendendo immediatamente riconoscibile l'esito di ogni operazione.
    \item \textbf{Colori Neutri e Tema Scuro:} Le variabili \texttt{--bs-body-bg} (\#121212) e \texttt{--card-bg} (\#2a2a2a) del tema \textit{dark} definiscono una stratificazione pensata per ridurre l'affaticamento visivo (\textit{eye-strain}). Il grigio antracite, preferito al nero assoluto, migliora la percezione di profondità tra i componenti e lo sfondo. L'effetto \textit{glassmorphism} (opacità all'85\%, es. \texttt{rgba(42, 42, 42, 0.85)}) aggiunge trasparenze parziali che creano continuità visiva tra i livelli dell'interfaccia.
\end{itemize}

\subsection{Tipografia e Leggibilità}
La dichiarazione globale (\texttt{\$font-family-sans-serif}) adotta la famiglia tipografica \textit{Outfit} (importata via \textit{Google Fonts}), seguita dal \textit{system stack} di fallback. Si tratta di un \textit{sans-serif} geometrico dalla buona leggibilità a qualsiasi dimensione di \textit{viewport}.

La gerarchia dei pesi è costruita attraverso le classi del framework (\texttt{display-4}, \texttt{h3}, \texttt{h5}), con un peso uniforme per i titoli (\texttt{\$headings-font-weight: 600;}). Questa impostazione favorisce lo \textit{scanning} rapido della pagina secondo il tipico F-Pattern, distinguendo chiaramente la \textit{hero section} dal corpo del contenuto testuale.

\subsection{Spaziature e Principi della Gestalt}
Le scelte di layout e spaziatura adottate in \textit{Guided Tours} applicano i principi della psicologia della forma (\textit{Gestalt}):
\begin{itemize}
    \item \textbf{Prossimità e Regione Comune:} L'uso sistematico di \texttt{.card} con bordi arrotondati (\texttt{border-radius: 1rem}) e sfondo differenziato (\texttt{--card-bg}) raggruppa visivamente gli elementi correlati in un'unica regione comune. Le spaziature regolari tra le card (\texttt{py-5}, \texttt{gap-2}) separano i gruppi logici distinti, sfruttando la \textit{Legge della Prossimità} per ridurre lo sforzo di lettura.
    \item \textbf{Continuità e Feedback visivo:} Le card presentano micro-transizioni al passaggio del mouse (\texttt{.card-hover:hover}): un leggero spostamento verticale (\texttt{translateY(-8px)}) accompagnato dall'intensificazione dell'ombra (\textit{drop-shadow}). Questo effetto di elevazione richiama le affordance degli oggetti fisici, suggerendo la cliccabilità dell'elemento e guidando l'esplorazione dell'utente secondo il principio di \textit{Buona Continuazione}.
\end{itemize}
...
\section{Role-Based Customization: Le 4 Homepage}
Per assecondare le diverse personas documentate in fase esplorativa, la Homepage (\texttt{home.blade.php}) agisce come smistatore univoco. Adatta infatti il suo layout e le proprie funzionalità in tempo reale basandosi sull'autenticazione tramite direttive di autorizzazione:
\begin{enumerate}
...
La Homepage rappresenta il punto di accesso principale all'applicativo. La nuova versione accoglie l'utente con un'hero section dal forte impatto visivo, un sottotitolo contestuale al ruolo dell'utente autenticato, un pannello di ricerca e filtraggio e una griglia di card tour ottimizzata (Figura~\ref{fig:v2_home}).
...
Le pagine di Login e Register nella V2 sono state semplificate e arricchite di meccanismi di supporto: toggle per la visibilità della password, validazione live dei campi e link diretto al recupero delle credenziali (Figura~\ref{fig:v2_auth}).
...
La Dashboard del Customer nella V2 mostra le prenotazioni attive con il booking code in evidenza, la codifica cromatica semantica degli stati visita e il pulsante di cancellazione con modale di conferma (Figura~\ref{fig:v2_dashboard}).
...
Il pannello amministrativo comprende il configuratore a schede (Places, Visit Types, Users), il pannello di Visit Planning con raggruppamento mensile e il form di creazione visite con flusso guidato step-by-step (Figure~\ref{fig:v2_admin_conf} e~\ref{fig:v2_planning}).
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.47\textwidth}
        ...
        \caption{Form Add Visit}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.47\textwidth}
        \centering
        \includegraphics[width=\textwidth, keepaspectratio]{capitoli/immagini/v2/v2_admin_planning.png}
        \caption{Visit Planning}
    \end{subfigure}
    \caption{V2 -- Form Add Visit con flusso guidato (sinistra) e Visit Planning raggruppato per mese (destra).}
    \label{fig:v2_planning}
\end{figure}

\subsection{Area Volontario (Guida)}

L'area riservata alle Guide presenta la pagina delle visite assegnate con banner operativo e il calendario per la dichiarazione delle disponibilità mensili (Figura~\ref{fig:v2_guide}).
