...
La Homepage funge da ambiente di accoglienza e snodo direzionale primario per le diverse tipologie di utenza.
Graficamente, il layout è dominato da una sezione introduttiva (\textit{Hero Section}) concepita per comunicare immediatamente la missione dell'organizzazione e instaurare un rapporto di fiducia, come visibile nella Figura~\ref{fig:home}.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{capitoli/immagini/v2/v2_home.png}
...
La gestione degli accessi rappresenta uno snodo critico nel percorso utente. Il modulo di registrazione, illustrato nella Figura~\ref{fig:login_register}, introduce una scelta logica dirimente sin dal primo contatto: la selezione del ruolo operativo.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{capitoli/immagini/v2/v2_login.png}
...
\end{itemize}
Al completamento del flusso, il sistema fornisce un feedback contestuale di presa in carico, ponendo la pratica nello stato di sospeso in attesa di validazione asincrona da parte dell'amministrazione (Figura~\ref{fig:booking}).

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{capitoli/immagini/v2/v2_dashboard.png}
...
L'obiettivo cognitivo di questo cruscotto, mostrato nella Figura~\ref{fig:availability}, è agevolare l'inserimento dei dati essenziali per il futuro algoritmo organizzativo, riducendo le frizioni e promuovendo l'efficienza.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{capitoli/immagini/v2/v2_guide_calendar.png}
...
L'interazione è modellata su griglie dati (pattern CRUD -- Create, Read, Update, Delete) pensate per le performance e l'accesso rapido all'informazione (Figura~\ref{fig:config_panel}):
\begin{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{capitoli/immagini/v2/v2_admin_conf.png}
...
Data l'elevata densità di dati, l'interfaccia utente è calibrata specificamente per supportare il \textit{decision making} dell'amministratore, mitigando l'overload informativo (Figura~\ref{fig:visit_planning}).
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{capitoli/immagini/v2/v2_admin_planning.png}
